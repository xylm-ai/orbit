...
export default function EquityPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([h, s]) => {
        setHoldings(h);
        const eq = s.entities.flatMap((e) => e.portfolios.filter((p) => p.portfolio_type === "equity"));
        setXirr(eq.find((p) => p.xirr != null)?.xirr ?? null);
      })
  ...
  return (
    <div>
      <h1 className="text-base font-bold text-slate-200 mb-5 tracking-tight">Direct Equity</h1>
      <StatStrip items={[
